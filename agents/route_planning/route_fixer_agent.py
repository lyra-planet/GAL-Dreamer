...
from prompts.route_planning.route_fixer_prompt import (
    ROUTE_FIXER_SYSTEM_PROMPT,
    ROUTE_FIXER_PROMPT
)
from utils.logger import log


class RouteFixerAgent(BaseAgent):
    """
    路线修复Agent
    ...
    name = "RouteFixerAgent"
    system_prompt = ROUTE_FIXER_SYSTEM_PROMPT
    human_prompt_template = ROUTE_FIXER_PROMPT
    ...
    def process(
        ...
        Args:
            ...
            issues: 检查报告中的问题列表
            fix_round: 当前修复轮次（用于多轮修复）

        Returns:
            修复后的主线框架
        """
        log.info(f"执行路线修复（第{fix_round}轮），共{len(issues)}个问题...")
        ...
        try:
            ...
            log.error(f"RouteFixerAgent 处理失败: {e}")
            raise RuntimeError(f"路线修复失败: {e}") from e
    ...
    def _get_fallback_response(self) -> Dict[str, Any]:
        """获取降级响应"""
        return {
            "structure_id": f"route_fixer_fallback_{uuid.uuid4().hex[:8]}",
            "state": {},
            "branches": [],
            "endings": [],
            "chapters": [],
